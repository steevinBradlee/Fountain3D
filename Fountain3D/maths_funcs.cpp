...
// rotate around local y axis by an angle in degrees
mat4 local_rotate_y(const mat4& m, float deg) {
	vec3 m_world_pos(m.m[12], m.m[13], m.m[14]);
	mat4 m_r = translate(m, inverse(m_world_pos));
	m_r = rotate_y_deg(m_r, deg);
	m_r = translate(m_r, m_world_pos);
	return m_r;
}
